...
export default function DriverHomePage({ isLoggedIn }) {
  ...
  useEffect(() => {
    if (isOnline && !activeRide && incomingRequests.length < 3) {
      const timer = setInterval(() => {
        setIncomingRequests(prev => [...prev, generateMockRideRequest()].slice(-3));
      }, 15000);
      return () => clearInterval(timer);
    }
  ...
  return (
    <div className="bg-gradient-to-br from-gray-50 to-emerald-50 min-h-screen font-sans">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">RideEase Driver</h1>
                <p className="text-sm text-gray-500">Multi-Location Expert</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <div className="relative group">
                <button className="p-2 rounded-full hover:bg-emerald-50 transition-colors relative">
                  <Bell className="h-6 w-6 text-gray-700" />
                  <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">3</span>
                </button>
                <div className="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded-xl shadow-xl p-4 hidden group-hover:block z-20 transform transition-all duration-300 scale-95 group-hover:scale-100">
              ...
              <div className="relative group">
                ...
                <div className="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded-xl shadow-xl p-4 hidden group-hover:block z-20 transform transition-all duration-300 scale-95 group-hover:scale-100">
                  <p className="text-sm font-semibold text-gray-900">{driverProfile?.name || 'Driver Name'}</p>
        ...
        <section className={`bg-white rounded-2xl shadow-xl p-6 mb-6 border-l-8 transition-all duration-300 ${isOnline ? 'border-emerald-500' : 'border-gray-300'}`}>
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <button
                    ...
                    className={`relative inline-flex items-center h-10 w-24 rounded-full transition-colors duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                        isOnline ? 'bg-emerald-500 focus:ring-emerald-500' : 'bg-gray-200 focus:ring-gray-500'
                    }`}
                >
                    <span className="sr-only">Toggle Online Status</span>
                    <span
                        // CHANGED: Reduced size and translation of the inner circle
                        className={`inline-block w-8 h-8 transform bg-white rounded-full transition-transform duration-300 ease-in-out shadow-lg ${
                            isOnline ? 'translate-x-[3.75rem]' : 'translate-x-1' // 60px
                        }`}
                    />
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-4">
                            <label className="flex items-center space-x-2 cursor-pointer">
                                <input type="checkbox" className="form-checkbox h-5 w-5 text-emerald-600 border-gray-300 rounded focus:ring-emerald-500" defaultChecked />
                                <span className="text-sm text-gray-800">Single Rides</span>
                            </label>
                            <label className="flex items-center space-x-2 cursor-pointer">
                                <input type="checkbox" className="form-checkbox h-5 w-5 text-purple-600 border-gray-300 rounded focus:ring-purple-500" defaultChecked />
                  ...
                  <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <div className="flex items-center space-x-3">
                      <div className={`w-2 h-2 rounded-full ${
                          passenger.status === 'dropped' ? 'bg-blue-500' :
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors transform hover:scale-105 ${
                    selectedRideType === 'all' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'
                  }`}
                >
                  All
                </button>
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors transform hover:scale-105 ${
                    selectedRideType === 'single' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'
                  }`}
                >
                  Single
                </button>
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors transform hover:scale-105 ${
                    selectedRideType === 'multi' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'
                  }`}
                >
                  Multi
                </button>
      ...
      {/* Online Notification */}
      {isOnline && (
        <div className="fixed top-20 right-4 bg-purple-600 text-white p-4 rounded-xl shadow-lg max-w-sm animate-slide-in">
          <div className="flex items-start space-x-3">
