        ...
        if (validateForm()) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    // ✅ FIX: The original code created a limited userData object here.
                    // The correction is to pass the entire 'data' object from the API response to onLogin.
                    // This ensures the App component gets all user details, including the profile image.
                    if (onLogin) {
                        // We combine the API response data with the role from our component's state.
                        const fullUserData = { ...data, role };
                        onLogin(fullUserData, data.token);
                    } else {
                        // This is fallback logic in case onLogin is not provided, which shouldn't happen
                        // For consistency, we ensure it creates a more complete object.
                        const fallbackUserData = {
                            _id: data._id,
                            name: data.name,
                            email: data.email,
                            role,
                            profileImageUrl: data.profilePicture || null,
                        };
                        localStorage.setItem('token', data.token);
                        localStorage.setItem('user', JSON.stringify(fallbackUserData));
                        setUser && setUser(fallbackUserData);
                        setIsLoggedIn(true);
                        navigate(role === 'user' ? '/' : '/driver/home'); // Corrected path
                    }
                } else {
