        ...
        const fetchMapData = async () => {
            ...
            try {
                ...
                const locationsToGeocode = [
                    ...debouncedPickups.map(loc => typeof loc === 'string' ? { address: loc, type: 'pickup' } : { address: loc.address, type: 'pickup', coord: loc.coord }),
                    ...debouncedDrops.map(loc => typeof loc === 'string' ? { address: loc, type: 'drop' } : { address: loc.address, type: 'drop', coord: loc.coord }),
                ...
                const coordsPromises = locationsToGeocode.map(loc =>
                    loc.coord 
                    ? Promise.resolve({ coord: loc.coord, pos: [loc.coord[1], loc.coord[0]], text: loc.address, type: loc.type })
